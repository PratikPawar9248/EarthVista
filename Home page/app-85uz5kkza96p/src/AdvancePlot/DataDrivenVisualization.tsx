...
import { 
  Cube, Eye, Brain, BookOpen, Play, Pause, RotateCcw, 
...
const FourDVisualization: React.FC<{
  data: OceanData[];
  parameter: string;
  timeStep: number;
  onLocationSelect: (location: OceanData) => void;
}> = ({ data, parameter, timeStep, onLocationSelect }) => {
  return (
    <div className="w-full h-full bg-gradient-to-br from-blue-900/20 to-purple-600/20 rounded-lg flex flex-col items-center justify-center p-6">
      <Cube className="w-16 h-16 mx-auto text-primary mb-4 animate-spin" />
      <div className="text-xl font-semibold text-foreground mb-2">4D Ocean Visualization</div>
      <div className="text-sm text-muted-foreground mb-4 text-center">
        ParaView-style 4D rendering with temporal evolution
      </div>
      <div className="grid grid-cols-3 gap-4 text-xs w-full max-w-md">
        <div className="bg-secondary/30 rounded-lg p-3 text-center">
          <div className="text-primary font-semibold">{parameter.toUpperCase()}</div>
          <div className="text-muted-foreground">Parameter</div>
        </div>
        <div className="bg-secondary/30 rounded-lg p-3 text-center">
          <div className="text-accent font-semibold">{timeStep}%</div>
          <div className="text-muted-foreground">Time Step</div>
        </div>
        <div className="bg-secondary/30 rounded-lg p-3 text-center">
          <div className="text-green-400 font-semibold">{data.length}</div>
          <div className="text-muted-foreground">Data Points</div>
        </div>
      </div>
      <div className="mt-4 text-xs text-muted-foreground text-center">
        Interactive 4D visualization with temporal animation and spatial rendering
      </div>
    </div>
  );
};

const ImmersiveVisualization: React.FC<{
  data: OceanData[];
  parameter: string;
  vrMode: boolean;
  arMode: boolean;
  onLocationSelect: (location: OceanData) => void;
}> = ({ data, parameter, vrMode, arMode, onLocationSelect }) => {
  return (
    <div className="w-full h-full bg-gradient-to-br from-cyan-900/20 to-blue-600/20 rounded-lg flex flex-col items-center justify-center p-6">
      {vrMode ? <Headphones className="w-16 h-16 mx-auto text-primary mb-4" /> :
       arMode ? <Glasses className="w-16 h-16 mx-auto text-accent mb-4" /> :
       <Eye className="w-16 h-16 mx-auto text-primary mb-4" />}
      
      <div className="text-xl font-semibold text-foreground mb-2">
        {vrMode ? 'Virtual Reality Ocean' : 
         arMode ? 'Augmented Reality Overlay' : 
         'Immersive 3D Environment'}
      </div>
      <div className="text-sm text-muted-foreground mb-4 text-center">
        {vrMode ? 'Full immersion in underwater environment with spatial audio' :
         arMode ? 'Real-world overlay with interactive data visualization' :
         'Interactive 3D ocean exploration with depth perception'}
      </div>
      <div className="grid grid-cols-2 gap-4 text-xs w-full max-w-md">
        <div className="bg-secondary/30 rounded-lg p-3 text-center">
          <div className="text-primary font-semibold">{parameter.toUpperCase()}</div>
          <div className="text-muted-foreground">Active Parameter</div>
        </div>
        <div className="bg-secondary/30 rounded-lg p-3 text-center">
          <div className="text-accent font-semibold">
            {vrMode ? 'VR' : arMode ? 'AR' : '3D'}
          </div>
