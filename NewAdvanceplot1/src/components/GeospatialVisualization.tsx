...
const RasterMap: React.FC<{ data: OceanData[], parameter: string, projection: string }> = ({ data, parameter, projection }) => {
  ...
  useEffect(() => {
    ...
    // Get parameter values for color mapping
    const values = data.map(d => d[parameter]).filter(v => v !== undefined && v !== null) as number[];
    ...
    for (let x = 0; x < width; x += gridSize) {
      for (let y = 0; y < height; y += gridSize) {
        ...
        const centerLat = 20 - (y / height) * 20; // 0-20°N
        const centerLon = 60 + (x / width) * 30; // 60-90°E
        
        const nearbyPoints = data.filter(point => 
          Math.abs(point.latitude - centerLat) < 2 && 
          Math.abs(point.longitude - centerLon) < 2 &&
          point[parameter] !== undefined
        );
        
        if (nearbyPoints.length > 0) {
          const avgValue = nearbyPoints.reduce((sum, p) => sum + (p[parameter] as number), 0) / nearbyPoints.length;
          const normalized = (avgValue - minVal) / (maxVal - minVal);
          
          // Color based on parameter
          let color = '';
          if (parameter === 'temperature') {
            color = `hsl(${240 - normalized * 240}, 70%, ${50 + normalized * 30}%)`;
          } else if (parameter === 'salinity') {
            color = `hsl(${120 - normalized * 120}, 70%, ${50 + normalized * 30}%)`;
          } else {
            color = `hsl(${60 + normalized * 60}, 70%, ${50 + normalized * 30}%)`;
          }
    ...
    ctx.fillText(`${parameter.toUpperCase()} Raster Map`, 10, 25);
    ctx.fillText(`Projection: ${projection}`, 10, 45);
    ctx.fillText(`Range: ${minVal.toFixed(2)} - ${maxVal.toFixed(2)}`, 10, 65);
    
  }, [data, parameter, projection]);
...
const ContourMap: React.FC<{ data: OceanData[], parameter: string, levels: ContourLevel[], projection: string }> = ({ data, parameter, levels, projection }) => {
  ...
  useEffect(() => {
    ...
    levels.forEach((level, index) => {
      const normalized = (level.value - minVal) / (maxVal - minVal);
      const radius = 50 + normalized * 100;
      
      // Draw multiple contour circles
      for (let i = 0; i < 5; i++) {
        const centerX = (width / 6) * (i + 1);
        const centerY = height / 2 + Math.sin(i) * 100;
        ...
        ctx.fillText(level.value.toFixed(1), centerX - 15, centerY - radius - 10);
      }
    });
    ...
    // Draw data points
    data.forEach(point => {
      if (point[parameter] === undefined) return;
      
      const x = ((point.longitude - 60) / 30) * width;
      const y = ((20 - point.latitude) / 20) * height;
      
      if (x >= 0 && x <= width && y >= 0 && y <= height) {
        const value = point[parameter] as number;
        const normalized = (value - minVal) / (maxVal - minVal);
        
        ctx.fillStyle = `hsl(${240 - normalized * 240}, 70%, 60%)`;
        ctx.beginPath();
    ...
    ctx.fillStyle = 'white';
    ctx.font = '12px monospace';
    ctx.fillText(`${parameter.toUpperCase()} Contour Map`, 10, 25);
    ctx.fillText(`Projection: ${projection}`, 10, 45);
    ctx.fillText(`Levels: ${levels.length}`, 10, 65);
    
  }, [data, parameter, levels, projection]);
